...
function setupWebSocketServer(httpServer) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('candidate-reorder', async (data) => {
      try {
        ...
        if (dragged && target) {
          await prisma.candidate.update({
            where: { id: data.draggedId },
            data: { sortOrder: target.sortOrder + 1 }
          })
        }
    ...
    // Видалення кандидата
    socket.on('candidate-delete', async (data) => {
      try {
        // Soft delete
        await prisma.candidate.update({
          where: { id: data.id },
...
function broadcastSystemMessage(message, room) {
  const io = global.io
  if (io) {
    if (room) {
